...
void check_disk_status() {
	...
	if (CLK >= disk_counter + 1024 && IOregisters[17]->data == 1) {// 1024 cycles have passed since read/write operation started
		...
		//printf("triggered irq1 in cycle %d\n", CLK); // DEBUG print
	}
}
...
int main(int argc, char* argv[]) {
	...
	while (1) { // fetch-decode-execute loop
		...
		//printf("CYCLE is %d, PC now is %d, instr is %s%s%s%s%s%s, ",CLK, PC, instr->op_code, instr->rd, instr->rs, instr->rt, instr->reservedBigimm, instr->imm8); // DEBUG print
		...
		if (read_result == TWO_ROW_INSTRUCTION) {
			...
			immediate = strtoul(instr->imm8, NULL, 16);
		...
		if (decode_res == HALT) {
			//printf("Reached end of asm program"); // DEBUG print
			break;
		}
