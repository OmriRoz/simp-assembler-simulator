...
void AssemblyToHex(char* line, char* mem_data[], int* instruction_counter, int label_counter, label label_table[]) {
	...
	if (strcmp(opcode_asm_buff, ".word") == 0) { // .word command
		...
		//printf("%s %s %s\n", opcode_asm_buff, para1_asm_buff, para2_asm_buff); // DEBUG print
		...
		//printf("%s %s, %s, %s, %s\n", opcode_asm_buff, para1_asm_buff, para2_asm_buff, para3_asm_buff, para4_asm_buff); // DEBUG print
...
int first_pass(FILE* asm, label label_table[]) {
	...
	while (!feof(asm)) {
		...
		if (strcmp(opcode_asm_buff, ".word") != 0) { // No need to increase ilc for .word operation
			ilc++;
